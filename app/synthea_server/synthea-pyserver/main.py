...
def extract_patient_info_for_pdf(patient_data: dict) -> dict:
    ...
    if addresses:
        ...
        result["personal_info"]["address"] = f"{line}, {city}, {state} {postal}".strip(", ")
    
    # Phone
    telecoms = patient.get("telecom", [])
    for tel in telecoms:
        if tel.get("system") == "phone":
            result["personal_info"]["phone"] = tel.get("value", "")
            break
...
def generate_patient_pdf(patient_info: dict, patient_id: str) -> bytes:
    ...
    # ===== SECTION 11: Medications (with prescriber info) =====
    ...
    if medications:
        ...
        med_data = [[P("Medication", header_cell_style), P("Status", header_cell_style), P("Date", header_cell_style), P("Prescriber", header_cell_style)]]
        for m in (active_meds + other_meds):
            med_data.append([
                P(m.get("name", ""), cell_style),
                P(m.get("status", ""), cell_style),
                P(m.get("authored_date", ""), cell_style),
                P(m.get("prescriber", ""), cell_style)
            ])
        
        t = Table(med_data, colWidths=[2.5*inch, 0.7*inch, 0.8*inch, 2*inch])
        t.setStyle(TableStyle([
    ...
    if encounters:
        ...
        enc_data = [[P("Visit Type", header_cell_style), P("Class", header_cell_style), P("Date", header_cell_style), P("Practitioner", header_cell_style), P("Provider/Location", header_cell_style)]]
        for e in encounters:
            ...
            enc_data.append([
                ...
                P(e.get("practitioner", ""), cell_style),
                P(location, cell_style)
            ])
        
        t = Table(enc_data, colWidths=[1.7*inch, 0.8*inch, 0.8*inch, 1.4*inch, 1.3*inch])
        t.setStyle(TableStyle([
    ...
    # ===== SECTION 16: Lab Results/Observations =====
    ...
    if observations:
        ...
        t = Table(obs_data, colWidths=[2.5*inch, 1*inch, 1.8*inch, 0.7*inch])
        t.setStyle(TableStyle([
    ...
    # ===== SECTION 17: Clinical Timeline (Key Events with Cross-References) =====
